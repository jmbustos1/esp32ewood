/**
 * @file sim7600.c
 * @brief Implementación del driver SIM7600 (AT, buffer mode, TCP, comandos servidor).
 */
...
/**
 * @brief Lee estado de conexiones con AT+CIPOPEN?
 * Respuesta: establecido = +CIPOPEN: <link>,<type>,<serverIP>,... ; no establecido = +CIPOPEN: <link>
 */
sim7600_response_t sim7600_cipopen_read(char *response, size_t response_size)
{
    const char cmd[] = "AT+CIPOPEN?\r\n";
    ESP_LOGI(TAG, "Leyendo estado CIPOPEN...");
    return sim7600_send_command(cmd, response, response_size, RESPONSE_TIMEOUT_MS);
}

/**
 * @brief Indica si en la respuesta de AT+CIPOPEN? el link está establecido.
 * Doc: establecido = +CIPOPEN: <link_num>,<type>,<serverIP>,... ; no establecido = +CIPOPEN: <link_num> solo.
 */
int sim7600_cipopen_is_link_established(int link_num, const char *cipopen_response)
{
    ...
    char pattern[24];
    snprintf(pattern, sizeof(pattern), "+CIPOPEN: %d,", link_num);
    return (strstr(cipopen_response, pattern) != NULL) ? 1 : 0;
}

/**
 * @brief Indica si el link está establecido y conectado al servidor (server_ip, server_port).
 * Formato doc: +CIPOPEN: <link>,"TCP","<serverIP>",<serverPort>,<index>
 */
int sim7600_cipopen_link_matches_server(int link_num, const char *cipopen_response, const char *server_ip, int server_port)
{
    if (!cipopen_response || !server_ip) {
        return 0;
    }
    char pattern[32];
    snprintf(pattern, sizeof(pattern), "+CIPOPEN: %d,\"TCP\",\"", link_num);
    const char *p = strstr(cipopen_response, pattern);
    if (!p) {
        return 0;
    }
    p += strlen(pattern);
    ...
    char ip[64];
    size_t iplen = (size_t)(q - p);
    memcpy(ip, p, iplen);
    ip[iplen] = '\0';
    int port = -1;
    if (sscanf(q + 1, ",%d", &port) != 1) {
        return 0;
    }
    return (strcmp(ip, server_ip) == 0 && port == server_port) ? 1 : 0;
}

/**
 * @brief Abre una conexión TCP en modo no transparente.
 * Usa AT+CIPOPEN? primero: si el link ya está conectado al servidor (app_config), no envía AT+CIPOPEN= (evita timeout).
 * Doc: <err> 0 = éxito, otro = fallo.
 */
sim7600_response_t sim7600_cipopen_tcp(int link_num, const char *server_ip, int server_port)
{
    char cmd[128];

    sim7600_response_t r = sim7600_cipopen_read(shared_response_buffer, sizeof(shared_response_buffer));
    if (r == SIM7600_OK && sim7600_cipopen_link_matches_server(link_num, shared_response_buffer, server_ip, server_port)) {
        ESP_LOGI(TAG, "✓ Link %d ya conectado a %s:%d (según AT+CIPOPEN?)", link_num, server_ip, server_port);
        return SIM7600_OK;
    }

    if (r == SIM7600_OK && sim7600_cipopen_is_link_established(link_num, shared_response_buffer)) {
        ESP_LOGI(TAG, "Link %d abierto a otro servidor, cerrando antes de reabrir...", link_num);
        sim7600_cipclose(link_num);
    ...
    if (result == SIM7600_OK) {
        ...
        if (cipopen_pos) {
            ...
            if (sscanf(cipopen_pos, "+CIPOPEN: %d,%d", &link, &err_code) == 2) {
                ...
                ESP_LOGE(TAG, "✗ Error en conexión: código %d (solo 0 es éxito)", err_code);
                return SIM7600_ERROR;
            }
        }
    }
 ...
 * Espera solo +CIPCLOSE (no CLOSED) para no devolver ERROR cuando el módulo responde +CIPCLOSE: n,4 + ERROR.
 */
sim7600_response_t sim7600_cipclose(int link_num)
{
    ...
    return sim7600_wait_for_response("+CIPCLOSE", shared_response_buffer, sizeof(shared_response_buffer), 10000);
}
...
void sim7600_scooter_update_loop(const char *server_ip, int server_port)
{
    ...
    // 0. Crear mutex UART lo antes posible (protege setup, recovery y loop)
    if (uart_mutex == NULL) {
    ...
    while (1) {
        ...
        /* Si no estamos en RUNNING, ejecutar recuperación y backoff */
        if (connectivity_get_state() != CONNECTIVITY_RUNNING) {
            connectivity_step_recovery(server_ip, server_port);
            connectivity_wait_backoff();
            continue;
        }

        /* Cada N ciclos: health-check AT (módulo sin respuesta) y estado de red */
        if (update_count % CONNECTIVITY_NET_CHECK_CYCLES == 0) {
            char at_health_buf[64];
            sim7600_response_t at_r = sim7600_send_command("AT\r\n", at_health_buf, sizeof(at_health_buf), RESPONSE_TIMEOUT_MS);
            if (at_r == SIM7600_TIMEOUT || at_r == SIM7600_ERROR) {
                connectivity_notify_at_timeout();
            } else {
                connectivity_notify_at_ok();
            }
            if (connectivity_get_state() != CONNECTIVITY_RUNNING) {
                continue; /* Pasamos a MODULE_OFFLINE (N timeouts); siguiente ciclo hará recovery */
            }
            char net_status[256];
            if (sim7600_netopen_status(net_status, sizeof(net_status)) == SIM7600_OK &&
                strstr(net_status, "+NETOPEN: 0") != NULL) {
                /* Red cerrada inesperadamente */
